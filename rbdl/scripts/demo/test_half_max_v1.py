...
if __name__ == '__main__':
    ...
    mu = device_put(0.9)
    print("contact_cond[contact_force_lb]",type(contact_cond["contact_force_lb"]))
    ...
    print("start")
    contact_force = dict()
    model["tau"] = jnp.zeros((7,))
    t = 0.0
    for i in range(10):
        # flag_contact_tuple = detect_contact(model, q, qdot)
        # print("flag_contact_tuple",flag_contact_tuple)
        # flag_contact_list = []
        ...
        #     idcontact, flag_contact, parent, jtype, jaxis, NB, NC, nf, rankJc, ncp, mu)
        xdot = dynamics_fun(t, x, model, contact_force)
        x = xdot + x
        print("xdot",xdot)
        print("x",x)
